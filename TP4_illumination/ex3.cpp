...
void lookAt(float* matrix, vec3 position, vec3 target, vec3 up) {
    ...
    forward = {forward.x / forward_length, forward.y / forward_length, forward.z / forward_length};

    vec3 s = {forward.y * up.z - forward.z * up.y, forward.z * up.x - forward.x * up.z, forward.x * up.y - forward.y * up.x};
    float s_length = sqrt(s.x * s.x + s.y * s.y + s.z * s.z);
    s = {s.x / s_length, s.y / s_length, s.z / s_length};

    vec3 u = {s.y * forward.z - s.z * forward.y, s.z * forward.x - s.x * forward.z, s.x * forward.y - s.y * forward.x};
...
struct Application {
    ...
    void Initialize() {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        unsigned char blueTexture[3] = {0, 0, 255}; // RGB pour bleu
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGB, GL_UNSIGNED_BYTE, blueTexture);
        glGenerateMipmap(GL_TEXTURE_2D);
    ...
    void Render() {
        ...
        float world[16];
        createWorldMatrix(world, translation, rotation, scale);
        int worldLoc = glGetUniformLocation(program, "world");
        glUniformMatrix4fv(worldLoc, 1, GL_FALSE, model);
